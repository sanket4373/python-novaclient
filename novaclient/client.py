...
class HTTPClient(object):
    ...
    def __init__(self, user, password, projectid=None, auth_url=None,
                 ...
                 proxy_token=None, region_name=None, email_id=None,
    ...
    def _cs_request(self, url, method, **kwargs):
	    ...
	    # Kerberos
            #kwargs.setdefault('headers', {})['X-Kerb-Serv-Tgt'] = self.kerberos_service_tgt
	    #kwargs.setdefault('headers', {})['X-Kerb-Serv-TimeStamp'] = self.kerberos_service_timestamp
            ...
            try:
                ...
                kwargs['headers']['X-Auth-Token'] = 'Hello'
                resp, body = self._time_request(url, method, **kwargs)
                return resp, body
	...
	# print "############# START KERBEROS PROCESSING ################"
        
	# User related Keys
	
 	#Ka = "alicekey"
 	#Ka = self.email_id
        #Ka_key = cms.cms_hash_data(Ka)
	#kerberos = respbody['access']['kerberos']
        #print("Kerberos ticket", kerberos)
      
	# User Ticket will be decrypted and session key 'Sa-Key' will also be obtained in this portion
	#print("Kerberos user ticket portion", kerberos['User_Ticket'])

	#decrypted_Ticket = cms.cms_decrypt_data(kerberos['User_Ticket'], Ka_key)
        #Sa_key = decrypted_Ticket['User_SessionKey']
	#print ("Decrypted Ticket -> ", decrypted_Ticket)
      
	# Service Ticket will be decrypted and user_service_sessionkey 'Kaos-Key' will also be obtained in this portion
        #print("Kerberos service ticket portion", kerberos['Service_Ticket'])
 
        #decrypted_service_Ticket = cms.cms_decrypt_data(kerberos['Service_Ticket'], Sa_key)
        #print ("Decrypted Service Ticket -> ", decrypted_service_Ticket)
	#Kaos_key = decrypted_service_Ticket['User_Service_SessionKey']
	
	#Service TGT extracted from the decode Service Ticket
	#kerberos_service_tgt = decrypted_service_Ticket['Service_TGT']
	#print("Service TGT -> ", decrypted_service_Ticket['Service_TGT'])		

	#self.kerberos_service_tgt = kerberos_service_tgt
	#self.kerberos_service_timestamp = cms.cms_encrypt_data(decrypted_service_Ticket['TimeStamp'], Kaos_key)
	#print ("Email is is here ->", email_id) 	
	#print "############# END KERBEROS PROCESSING ################"
...
def _construct_http_client(username=None, password=None, email_id=None, project_id=None,
